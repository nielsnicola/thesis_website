...
const scrollGif = document.querySelector("#scrollGif");

if (!container || !scrollGif) {
  console.warn("Missing #container or #scrollGif");
}
...
function animate() {
    ...
    // move text
    container.style.transform = `translateY(${-current * 0.025}px)`;

    const maxScroll = document.body.scrollHeight - window.innerHeight;

    const progress = maxScroll > 0
        ? Math.max(0, Math.min(1, current / maxScroll))
        : 0;

    const frameIndex = Math.floor(progress * (totalFrames - 1));

    if (frames[frameIndex] && scrollGif) {
        scrollGif.src = frames[frameIndex].src;
    }
